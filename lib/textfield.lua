...
return function (options)
  ...
  function textfield:update()
    ...
    local selection = function (text) return '<span background="'..gears.color.ensure_pango_color(beautiful.bg_selection)..'">'..gears.string.xml_escape(text)..'</span>' end
  ...
  textfield.keygrabber = options.keygrabber or awful.keygrabber({
    ...
    stop_callback = function (_, key)  end,
  })
  ...
  textfield.keygrabber.keypressed_callback = function (_, mod, key)
    ...
    elseif (key == 'Left') then
      if (not gears.table.hasitem(mod, 'Shift') and textfield.position.head ~= textfield.position.tail) then
        ...
        local pos = gears.table.hasitem(mod, 'Control') and textfield.value:sub(0, textfield.position.head):find('[%a%d_]*[^%a%d_]*$') or textfield.position.head
    ...
    elseif (key == 'Right') then
      if (not gears.table.hasitem(mod, 'Shift') and textfield.position.head ~= textfield.position.tail) then
        ...
        local offset = gears.table.hasitem(mod, 'Control') and table.pack(textfield.value:sub(textfield.position.head + 1, -1):find('^[^%a%d_]*[%a%d_]*'))[2] or 1
    ...
    elseif (key == 'x' and gears.table.hasitem(mod, 'Control')) then
      clipboard = textfield.value:sub(textfield.position.head + 1, textfield.position.tail)
      replaceAt(textfield.position.head, textfield.position.tail, '')
      if (options.input_callback) then options.input_callback(textfield.value) end
